...
class Page
    ...
    #retrieves the info of a page
    def get_page_info()
      result = @site.query_prop_info(@normtitle)
      return result['query']['pages']['page']
    end
    ...
    def get()
        ...
        elsif result['query']['pages']['page'].key? 'missing'
    ...
    def get_interwikis(min = nil, lllimit = 500)
        ...
        count = 0
        loop {
            result = @site.query_prop_langlinks(@normtitle, llcontinue)
            if result['query']['pages']['page'].key?('missing')
                raise NoPage.new(), "Page [[#{title}]] does not exist"
            end
            if !result['query']['pages']['page'].key?('langlinks')
                return nil
            end
            langlinks += result['query']['pages']['page']['langlinks']['ll']
#            if result['query']['pages']['page']['langlinks']['ll'].is_a? Array
#                langlinks = langlinks + result['query']['pages']['page']['langlinks']['ll']
#            else
#                langlinks.push(result['query']['pages']['page']['langlinks']['ll'])
#            end
...
#    #request elements until it has at least min elements.
#    #returns false if there aren't any, and raises NoPage if page doesn't exist
    def get_embeddedin(param=nil)
      param = Hash.new() unless param
      eis = []
      loop {
          result = @site.query_list_embeddedin(@normtitle, @normtitle, param[:eicontinue], param[:einamespace], param[:eifilterredir], param[:eilimit]='max')

          if result['query']['embeddedin'].count > 0
            eis += result['query']['embeddedin']['ei']
          end

          if result.key?('query-continue')
              param[:eicontinue] = result['query-continue']['embeddedin']['eicontinue']
          else
              break
          end
      }
